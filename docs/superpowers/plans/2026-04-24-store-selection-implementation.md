...
`/Users/yo-chan/アプリ/barcode-order/test.html` の `// --- csv ---` セクションの**直前**に、以下のブロックを挿入する（直前の `assertEq(storage.isSupplierInUse("Amazon"), true, "supplier in use: by order line");` の後）：
...
`/Users/yo-chan/アプリ/barcode-order/storage.js` の `KEYS` 定数を以下のように更新（`token` の後に `stores` と `current_store` を追加）：
...
import('/Users/yo-chan/アプリ/barcode-order/storage.js').then(s => {
...
`/Users/yo-chan/アプリ/barcode-order/test.html` 内の以下の行を見つける：
...
`/Users/yo-chan/アプリ/barcode-order/csv.js` の以下の関数を：
...
```bash
node --check /Users/yo-chan/アプリ/barcode-order/csv.js && echo "csv.js syntax OK"
```
...
`/Users/yo-chan/アプリ/barcode-order/index.html` の以下の `<header>` ブロックを：
...
`/Users/yo-chan/アプリ/barcode-order/styles.css` の以下のヘッダー定義を見つける：
...
`/Users/yo-chan/アプリ/barcode-order/app.js` の `renderAuthStatus` 関数の**直前**（`// --- 認証 UI ---` セクションの前）に以下を挿入：
...
`/Users/yo-chan/アプリ/barcode-order/app.js` の以下の関数を：
